...
images = glob.glob(os.path.join(
    os.getcwd(), 'Classes', '**', '*.png'), recursive=True)
...
if not os.path.isdir(annotation_path):
    os.mkdir(os.path.join(os.getcwd(), "Annotations"))
    annotation_path = os.path.join(os.getcwd(), "Annotations")

if not os.path.isdir(os.path.join(os.getcwd(), "JPEGImages")):
    os.mkdir(os.path.join(os.getcwd(), "JPEGImages"))
...
i = 1
path = os.path.join(os.getcwd(), "backgrounds")
num_objects = len(images) if len(images) <= 5 else 5
...
while i < numImages:
    ...
    if '.DS_Store' not in random_filename:
        img_bg = Image.open(os.path.join(path, random_filename))
        width, height = img_bg.size

        # Randomly determine number of objects to put on image
        a = random.randint(1, num_objects)
