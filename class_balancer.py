...
class ClassBalancer:
    ...
    def get_balancing_stats(self, balance_threshold = 0.00):
        balancing_base = min(self.class_distribution.items(),key = lambda x:x[1])
        ...
        for k,v in self.class_distribution.items():
            ...
            if(v > allowed_thresh*balancing_base[1]):
                ...
                # balancing_labels.append(k)
            else:
                balanced_classes[k] = v
                # balanced_labels.append(k)

        # print("balancing classes",balancing_classes)
        # print("balancing labels",balancing_labels)
        ...
        while len(balancing_classes) != 0:
            if len(class_fnames_list) != 0:
                ...
                # print("selected frame", frame_data)
                # print("balanced_classes",balanced_classes)
                # print("balancing_classes",balancing_classes)
...
#If successfully achieved required balancing 
    ...
    def partition_balanced_dataset(self, to_move_list, **kwargs):
        ...
        for i in to_move_list:
            y,_exten = i.split('.')
            # print(y,_)
            dest = shutil.move(self.path_annotations + '/' + y + '.' + _exten,self.path_holdout_annotations)
            dest = shutil.move(self.path_images + '/' + y + '.' + 'jpg',self.path_holdout_images)
...
input_dir = '3_cam/all-side-cams-merged-dataset'
...
print("To be moved list", len(move_list))

# balancer.partition_balanced_dataset(move_list)
